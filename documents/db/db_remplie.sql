...
--
-- Temporary view structure for view `populars_authors`
--

DROP TABLE IF EXISTS `populars_authors`;
/*!50001 DROP VIEW IF EXISTS `populars_authors`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `populars_authors` AS SELECT 
 1 AS `notation`,
 1 AS `id`,
 1 AS `firstname`,
 1 AS `lastname`,
...
INSERT INTO `users` VALUES (1,'charles_dubois@example.com','$2y$10$LBYExGHXa6kcIqOaXEYebeoAFRPubX/gayaoWxQxzrdhvVuA.YOka','Charles','2023-05-31 12:48:59'),(2,'elise_martin@example.com','$2y$10$ZUGqvxXZ7N8Wucs2ElWH6uUsInJjnPhphX3NeC3p9277qG6D30tRC','Elise','2023-05-31 12:48:59'),(3,'pierre_bernard@example.com','$2y$10$Mtng/4tTyJ0MP9Um3YHyjeaSUyOe3l4/FcUGg9RdvI/4L1x7NnpSa','Pierre','2023-05-31 12:48:59'),(4,'sophie_dumont@example.com','$2y$10$tLAdh/ufjPY/YDWKFCvoaOqOxaYfDowYO.jpCanxuq/EeEkvgFNY.','Sophie','2023-05-31 12:48:59'),(5,'lucas_moreau@example.com','$2y$10$IlkMCiSJaeUK4gDxh670D.q3X6wEUdS2hu59lysKg5c8VjMZeu1iO','Lucas','2023-05-31 12:48:59'),(6,'chloe_lemoine@example.com','$2y$10$QbCV1IxdYpUDndPafVC9hOVVyup/tyIrYnHJBOoDB8BaC3xl8KxH6','Chloe','2023-05-31 12:48:59'),(7,'alexandre_garnier@example.com','$2y$10$R7F3OqfxxNUOsUwqbhnRNOc8KGL6jzeW1kASAMYAGgdS9lMix2hmK','Alexandre','2023-05-31 12:48:59'),(8,'julie_rodriguez@example.com','$2y$10$mrnq8xeFKZK67W67TAlEKOeQ88kAfV41jiAdLkTj.TgdfSioZcPwi','Julie','2023-05-31 12:48:59'),(9,'thomas_legrand@example.com','$2y$10$hLYskbNOrs7P3Cd7HEnRPOFxIEF8TI3JNM1atIQuMBhdHrkhz678O','Thomas','2023-05-31 12:48:59'),(10,'emilie_gauthier@example.com','$2y$10$12OPWFSD.k.00PDJujMxkeUqT6trWPIumKpwTp671OJT2jL2.SvLK','Emilie','2023-05-31 12:48:59'),(11,'nicolas_perez@example.com','$2y$10$Y/tKz6jljJN11E/1r8S9B.5o2vpXKMXP9Q9GawCMeHOBX3hF3iPlq','Nicolas','2023-05-31 12:48:59'),(12,'camille_blanc@example.com','$2y$10$dhYWJmFCeAglkArvGjWW8eRp5LKMu4THJJXl9oyozRAZX6hloKpb2','Camille','2023-05-31 12:48:59'),(13,'maxime_petit@example.com','$2y$10$UHnKtBHfCVdfInCgTt/DNeMsyhGMukNyP7v9lkWzjndOsKRtoCU0C','Maxime','2023-05-31 12:48:59'),(14,'marie_morel@example.com','$2y$10$IY307LFFz797pnzaDnt7IOItLcZfb6f2kMIEbLPhsB4zfZgoU.O42','Marie','2023-05-31 12:48:59'),(15,'adrien_boucher@example.com','$2y$10$8KO3C8BPRDgzihbRE0gNc.m03WMR/NDK1d8IgAO0unhUERQAyzX72','Adrien','2023-05-31 12:48:59'),(16,'celine_brun@example.com','$2y$10$q/R6Rbzo3LCE83Qv2.ENM.5uCmRtVVgLBq3R91jmXC3Lcb9b6KgGq','Celine','2023-05-31 12:48:59'),(17,'vincent_rousseau@example.com','$2y$10$qxr1ECQAty1WyeGVDCu3h.QaQ7Fe27gPssnLq2h1FEQLWbdhYtY7i','Vincent','2023-05-31 12:48:59'),(18,'claire_giraud@example.com','$2y$10$uQQeRtkw.Bk9l1IBkI3PXOX8zrKTXj3x4t9kBvB..FQf/HNZTPWxi','Claire','2023-05-31 12:48:59'),(19,'françois_marchand@example.com','$2y$10$tALp2TpMuPFuFN5DaPKgEuyDqGLv57kkPGKaQzIZ6w2PRZeHgsL3a','François','2023-05-31 12:48:59'),(20,'jessica_lefevre@example.com','$2y$10$Pc57lJkdZMuUEs69u1uP9uIiYsEqHvDPMrflknwg7GPRyBMYXRMXC','Jessica','2023-05-31 12:48:59');
...
--
-- Final view structure for view `populars_authors`
--
...
/*!50001 VIEW `populars_authors` AS select round(avg(`un`.`note`),2) AS `notation`,`au`.`id` AS `id`,`au`.`firstname` AS `firstname`,`au`.`lastname` AS `lastname`,`au`.`biography` AS `biography`,`au`.`picture` AS `picture`,`au`.`created_at` AS `created_at` from ((`books` `bk` join `authors` `au` on((`bk`.`author_id` = `au`.`id`))) left join `users_notations` `un` on((`un`.`book_id` = `bk`.`id`))) group by `au`.`id` order by `notation` desc limit 3 */;
...
-- Dump completed on 2023-09-25 14:07:05
